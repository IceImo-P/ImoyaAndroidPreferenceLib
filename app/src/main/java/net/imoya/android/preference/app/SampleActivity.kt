...
class SampleActivity : AppCompatActivity(), DialogListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val parent: PreferenceScreenParent = PreferenceScreenParentActivity(this)
        string1Editor = StringPreferenceEditor(parent, preferences, REQUEST_STRING_1)
        number1Editor = NumberAndUnitPreferenceEditor(parent, preferences, REQUEST_NUMBER_1)
        number2Editor = SliderNumberEditor(parent, preferences, REQUEST_NUMBER_2)
        intList1Editor = IntListPreferenceEditor(parent, preferences, REQUEST_INT_LIST_1)
        stringList1Editor = StringListPreferenceEditor(parent, preferences, REQUEST_STRING_LIST_1)
        time1Editor = TimePreferenceEditor(parent, preferences, REQUEST_TIME_1)
        timePeriod1Editor = TimePeriodPreferenceEditor(parent, preferences, REQUEST_TIME_PERIOD_1)
        val switchController = SwitchPreferenceViewController(preferences)

        if (savedInstanceState != null) {
            AppLog.v(TAG, "onCreate: restoring editor state")
        ...
        PreferenceViewUtil.setupView(findViewById(R.id.pref_string_1), string1Editor, views)
        PreferenceViewUtil.setupView(findViewById(R.id.pref_number_1), number1Editor, views)
        PreferenceViewUtil.setupView(findViewById(R.id.pref_number_2), number2Editor, views)
        PreferenceViewUtil.setupView(findViewById(R.id.pref_switch_1), switchController, views)
        PreferenceViewUtil.setupView(
            findViewById(R.id.pref_string_list_1),
            stringList1Editor,
            views
        )
        PreferenceViewUtil.setupView(findViewById(R.id.pref_int_list_1), intList1Editor, views)
        PreferenceViewUtil.setupView(findViewById(R.id.pref_time_1), time1Editor, views)
        PreferenceViewUtil.setupView(
            findViewById(R.id.pref_time_period_1),
            timePeriod1Editor,
            views
        )
        ...
        findViewById<PreferenceView>(R.id.back).onPreferenceViewClickListener =
            object : OnPreferenceViewClickListener {
    ...
    override fun onResume() {
        ...
        AppLog.v(TAG, "onResume: start")

        updater.update(preferences)
        updater.start(preferences)

        AppLog.v(TAG, "onResume: end")
    }

    override fun onPause() {
        super.onPause()

        AppLog.v(TAG, "onPause: start")

        updater.stop(preferences)

        AppLog.v(TAG, "onPause: end")
    }
