...
/**
 * 設定値が整数値である、 [MultiSelectionListPreferenceView] の実装
 *
 * 設定項目風の [View] です。
 ...
 *  # カスタムレイアウト
 *
 * レイアウトXMLへ [MultiSelectionIntListPreferenceView] を配置する際、
 * android:layout([android.R.attr.layout])
 * 属性を指定することで、自由に定義されたレイアウトを適用することができます。
 *
 * 適用するレイアウトは [PreferenceView] の説明に記載された規則に従ってください。
 */
open class MultiSelectionIntListPreferenceView : MultiSelectionListPreferenceView {
    /**
     * 再起動時に保存する状態オブジェクト定義
     */
...
//    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    ...
    override fun getCheckedList(sharedPreferences: SharedPreferences): BooleanArray {
        ...
        selectionString.split(',')
            .filter { it.isNotEmpty() }.map { it.trim() }.map { it.toInt() }
            .forEach {
