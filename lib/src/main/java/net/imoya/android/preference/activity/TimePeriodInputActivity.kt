...
open class TimePeriodInputActivity : BaseActivity() {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        if (item.itemId == R.id.ok) {
            ...
            if (state!!.inputStep == InputStep.END) {
                PreferenceLog.v(TAG, "onOptionsItemSelected: Complete")
                ...
                PreferenceLog.v(TAG, "onOptionsItemSelected: to EndTime")
    ...
    override fun onSupportNavigateUp(): Boolean {
        return if (this.supportFragmentManager.backStackEntryCount == 0) {
            PreferenceLog.v(TAG, "onSupportNavigateUp: Cancel")
            finish()
            true
        } else {
            // 終了時刻入力中か?
            if (state!!.inputStep == InputStep.END) {
                PreferenceLog.v(TAG, "onSupportNavigateUp: to StartTime")
